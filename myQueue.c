...
static int __init myQueue_init(void) {
    ...
    if (IS_ERR(myQueueClass)) {
        ...
        printk(KERN_ALERT "myQueue: Failed to register device class\n");
        return PTR_ERR(myQueueClass);
    }
    printk(KERN_INFO "myQueue: device class registered\n");
    ...
    if (IS_ERR(myQueueDevice)) {
        ...
        unregister_chrdev(majorNumber, DEVICE_NAME);
        printk(KERN_ALERT "myQueue: Failed to create the device\n");
        return PTR_ERR(myQueueDevice);
    }
    printk(KERN_INFO "myQueue: device class created\n");
...
static void __exit myQueue_exit(void) {
    ...
    class_destroy(myQueueClass);
    unregister_chrdev(majorNumber, DEVICE_NAME);
    printk(KERN_INFO "myQueue: ended and delated the device\n");
}
...
static ssize_t dev_read(struct file* filep, char* buffer, size_t len, loff_t* offset) {
    ...
    if (queue_count > 0) {
        ...
        if (error_count == 0) {
            printk(KERN_INFO "myQueue: Sent 1 characters to the user\n");
            return 1;
        } else {
...
static ssize_t dev_write(struct file* filep, const char* buffer, size_t len, loff_t* offset) {
    ...
    if (queue_count == QUEUE_SIZE) {
        printk(KERN_INFO "myQueue: Queue is full in non-blocking mode\n");
        return -EAGAIN;
    }
    ...
    if (queue_count < QUEUE_SIZE) {
        ...
        if (error_count == 0) {
            ...
            printk(KERN_INFO "myQueue: Received 1 characters from the user\n");
            return 1;
        } else {
